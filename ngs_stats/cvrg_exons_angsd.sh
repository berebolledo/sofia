...
cvrg_50=$(zcat ${out}.pos.gz|awk 'NR>1 && $3>=50 {sum+=1} END{print sum}')
cvrg_100=$(zcat ${out}.pos.gz|awk 'NR>1 && $3>=100 {sum+=1} END{print sum}')
not_cvrg=$(expr 80457039 - $cvrg_1)

mean_cvrg=$(zcat ${out}.pos.gz|awk 'NR>1 {sum+=$3} END{print sum/80457039}')
...
rm -f ${out}.metrics
touch ${out}.metrics
printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" sample nreads cvrg1 cvrg50 cvrg100 mean_cvrg median_cvrg bp_over_mean bp_over_median >> ${out}.metrics
printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" $out $nreads $cvrg_1 $cvrg_50 $cvrg_100 $mean_cvrg $median_cvrg $over_mean $over_median >> ${out}.metrics
